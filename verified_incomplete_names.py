'''
Verified profiles without complete last name or first name
'''
import base
...
def main():
    ...
    more = True
    while more:
        base_url = 'https://api.humanitarian.id/api/v2/user?verified=true&sort=name&limit='+str(LIMIT)+\
    ...
    # Update Google Sheet
    try:
        worksheet = base.wks.add_worksheet(title="Verified - Incomplete", rows=(len(ids)+4), cols=10)
        worksheet.update_acell('A2', "Verified profiles with potentially incomplete names")
        worksheet.update_acell('A3', "User ID")
        worksheet.update_acell('B3', "Given Name")
        worksheet.update_acell('C3', "Family Name")
    except APIError:
        worksheet = base.wks.worksheet("Verified - Incomplete")
