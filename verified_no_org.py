'''
Verified profiles without organization (no auth users)
'''
import base
...
def main():
    ...
    # Update Google Sheet
    try:
        worksheet = base.wks.add_worksheet(title="Verified - No Org", rows=(len(ids)+4), cols=10)
        worksheet.update_acell('A2', "Verified users without organization (excludes auth users)")
        worksheet.update_acell('A3', "User ID")
        worksheet.update_acell('B3', "Given Name")
        worksheet.update_acell('C3', "Family Name")
    except APIError:
        worksheet = base.wks.worksheet("Verified - No Org")
